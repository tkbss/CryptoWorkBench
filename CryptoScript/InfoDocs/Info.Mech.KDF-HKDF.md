...
KDF-HKDF derives output keying material (OKM) using the complete HMAC-based Extract-and-Expand Key Derivation Function specified in RFC 5869. Extract first derives a pseudorandom key (PRK) from the input keying material (IKM), then Expand derives the requested OKM. Use HKDF-EXTRACT or HKDF-EXPAND when only one of these two operations is required.
