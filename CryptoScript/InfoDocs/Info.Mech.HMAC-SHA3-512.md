# MECHANISM HMAC-SHA3-512

HMAC-SHA3-512 calculates a keyed message authentication code using the HMAC construction and the SHA3-512 hash function. It supports MAC calculation rather than encryption or decryption.

---

## Key Features

- **Hash Function**: SHA3-512 belongs to the SHA-3 family, produces a 512-bit digest and is specified in FIPS 202. HMAC is specified in RFC 2104 and FIPS 198-1; RFC 9688 defines HMAC-SHA3 conventions for CMS.
...
- **MAC Length**: Mac always returns the complete 64-byte HMAC-SHA3-512 tag.
- **Return Value**: Mac returns a VAR containing hexadecimal MAC bytes in 0x(...) format.

## Functions
The following functions are available for HMAC-SHA3-512 mechanism:

| Functions | Parameters | Input | Output | Key Length |
|------------|-------------------|-------------------------------------|----------------|------------------|
| GenerateKey | N/A | HMAC-SHA3-512, positive byte-aligned key length or hexadecimal key | KEY variable | Positive multiple of 8 bits |
| Parameters | N/A | #MECH | PARAM variable | N/A |
| Mac | #MECH | parameters, key, data | VAR MAC | Any non-empty key |

## Parameters
These parameters are used with the HMAC-SHA3-512 mechanism:
- **#MECH**: Specifies HMAC-SHA3-512 in PARAM.
...
---
## Example Usage
### HMAC-SHA3-512 MAC Calculation
KEY k0         = GenerateKey(HMAC-SHA3-512, 512)  
PARAM p0       = Parameters(HMAC-SHA3-512)  
VAR cleartext  = "This is a secret message."  
VAR mac        = Mac(p0, k0, cleartext)  
### HMAC-SHA3-512 Empty Input
