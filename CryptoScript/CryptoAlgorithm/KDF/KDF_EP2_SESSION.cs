...
public sealed class KDF_EP2_SESSION : CryptoAlgorithm
{
    private const string MechanismName = "KDF-EP2-SESSION";
    private const string HashMechanism = "HASH-SHA256";

    private sealed record VariantDefinition(byte[] Info, int OutputLengthBytes);

    private static readonly IReadOnlyDictionary<string, VariantDefinition> Variants =
        new Dictionary<string, VariantDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ["TC"] = new(Convert.FromHexString("7184718471847184"), 32),
            ["MAC-SEND"] = new(Convert.FromHexString("EB5AEB5AEB5AEB5A"), 32),
            ["MAC-RECEIVE"] = new(Convert.FromHexString("9309930993099309"), 32),
            ["ENCRYPTION"] = new(Convert.FromHexString("0C7E0C7E0C7E0C7E"), 16),
            ["PIN"] = new(Convert.FromHexString("C5B1C5B1C5B1C5B1"), 16),
            ["KEY-ENCRYPTION"] = new(Convert.FromHexString("3FE73FE73FE73FE7"), 16)
        };
    ...
    public override ParameterVariableDeclaration GenerateParameters(string mechanism, string[] parameters)
    {
        ...
        if (!values.TryGetValue("#VARIANT", out string? variant) || !Variants.ContainsKey(variant))
            throw new ArgumentException($"{MechanismName} requires #VARIANT with a supported session-key variant.");
        string canonicalVariant = Variants.Keys.Single(candidate =>
            candidate.Equals(variant, StringComparison.OrdinalIgnoreCase));

        var result = new ParameterVariableDeclaration { Mechanism = MechanismName };
        result.SetParameter("SALT", salt);
        result.SetParameter("VARIANT", canonicalVariant);
    ...
    public override KeyVariableDeclaration Derive(string[] parameters)
    {
        ...
        string variant = parameter.GetParameter("VARIANT");
        if (!Variants.TryGetValue(variant, out VariantDefinition? definition))
            throw new ArgumentException($"{MechanismName} #VARIANT is not a supported session-key variant.");

        byte[] prk = HKDFMode.Extract(HashMechanism, salt, sessionKey);
        byte[] output = HKDFMode.Expand(HashMechanism, prk, definition.Info, definition.OutputLengthBytes);
    ...
    private static byte[] ResolveKeyBytes(KeyVariableDeclaration key)
    {
        ...
        if (!IsDataFormat(format))
            throw new ArgumentException($"{MechanismName} session key must contain a valid hex, Base64 or string value.");
        return ConvertToBytes(value, format, $"{MechanismName} session key");
    }
